...
widthP = Math.min(
    parseInt(getComputedStyle(sizer).getPropertyValue("width")) / 600,
)
var Inc = orInc
...
if (localStorage.getItem("place7896") == null) {
    ...
    var storedSearches = [
        JSON.parse(localStorage.place7896),
        JSON.parse(localStorage.levels7896),
    ]
    displaySearches(fs)
}
let startClick
...
function myFunction() {
    clicked = true
    if (
        /^\d{5}(-\d{4})?$/.test(locationInput.value) &&
        locationInput.value.length == 5
    )
        changeZip(locationInput.value)
    else if (
        locationInput.value == undefined ||
        locationInput.value.length < 2 ||
        /^\d+$/.test(locationInput.value)
    ) {
        locationInput.value = ""
...
//Initial grid draw.
function drawGrid(lati, lonj, s, City) {
    //'s' is width and height of grid.
    locationInput.value = ""
            ...
            fetch(pollutionUrl)
                ...
                .then(function () {
                    ...
                    if (p == 81) {
                        ...
                        searchCurrentEl.removeEventListener(
                            "click",
                            searchCurrent,
                        )
                        ...
                        disp.children[0].addEventListener(
                            "mousedown",
                            function () {
                                isMouseDown = true
                            ...
                            map.forEachFeatureAtPixel(
                                ...
                                function (feature, layer) {
                                    ...
                                    let canvasContext = document
                                        ...
                                        .getContext("2d")
                                    pixelAtCoords = canvasContext.getImageData(
                                        pixelCoords[0],
                                        pixelCoords[1],
                                        1,
                                        1,
                        ...
                        map.on("click", function (evt) {
                            ...
                            map.forEachFeatureAtPixel(
                                ...
                                function (feature, layer) {
                                    ...
                                    //Grabs coordinates of selected feature and sets popup to center coordinates.
                                    let clickedCoordinate =
                                        feature.A.geometry.flatCoordinates
                                    let clickedInfo = feature.get("id")
                                    popupInfo.innerHTML = clickedInfo
                                    popup.setPosition(clickedCoordinate)
                                    oldStyle = oldFill
                        ...
                        map.once("rendercomplete", function () {
                            for (let each in features) {
                                ...
                                let coords = features[each]
                                    .getGeometry()
                                    .getCoordinates()
                                if (isWater(coords))
                                    vectorSource.removeFeature(features[each])
                            }
                            if (ogLat != lati && ogLon != lonj)
                                currentLoc.setAttribute(
                                    "style",
                                    "visibility: visible",
                                )
                            clicked = false
...
function restoreSearchButton() {
    if (
        isMouseDown == true ||
        isMouseDown == undefined ||
        (isScrolling == true && searched == true)
    ) {
        searchCurrentEl.setAttribute("style", "visibility: hidden")
        currentLoc.setAttribute("style", "visibility: hidden")
    ...
    if (
        (isMouseDown == false || isMouseDown == undefined) &&
        isScrolling == false
    ) {
        clearTimeout(T)
...
function searchLocation(search) {
    ...
    getLocationData(search).then(function (data) {
        ...
        if (box[2] > 0 && box[0] > 0) {
            ...
            ;(bottom = Math.max(box[0], data.lat - 15)),
                (top = Math.min(box[1], data.lat + 15))
            ;(left = Math.max(box[2], data.lon - 15)),
                (right = Math.min(box[3], data.lon + 15))
            goToLocation(data.lat, data.lon, [left, bottom], [right, top])
            ...
            if (box[3] - box[2] > 100) {
                ...
                ;(bottom = Math.max(box[0], data.lat - 7)),
                    (top = Math.min(box[1], data.lat + 7))
                ;(left = Math.max(box[2], data.lon - 7)),
                    (right = Math.min(box[3], data.lon + 7))
                goToLocation(data.lat, data.lon, [left, bottom], [right, top])
            }
        } else {
            ;(left = box[2]),
...
function changeZip(zipCode) {
    ...
    fetch(zipUrl)
        ...
        .then(function (data) {
            ...
            lat = data[0].lat
            lon = data[0].lon
            city = data[0].display_name.substring(
                0,
                data[0].display_name.indexOf(","),
            )
    ...
    fetch(locUrl)
        ...
        .then(function (data) {
            try {
                ...
                city = data.display_name.substring(
